...
class INBODY(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        inputLayout.addWidget(self.showB, 0, 6, 2, 1)
        ...
        self.rankB = QPushButton("rank", self)
        self.rankB.setMaximumHeight(100)
        outputLayout.addWidget(self.rankB, 0, 1)
        self.rankB.clicked.connect(self.rankC)
...
class MyWindow(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Rank exercise")
        self.setGeometry(400, 400, 400, 400)
        rankLayout = QGridLayout()
        rankLayout.setSizeConstraint(QLayout.SetFixedSize)
        ...
        self.textTreeview.setAlternatingRowColors(True)
        # 헤더(목록)설정
        self.resultTreeview = QStandardItemModel(0, len(RankHeader), self)
        for i in range(len(RankHeader)):
            self.resultTreeview.setHeaderData(i, Qt.Horizontal, RankHeader[i])
        # 내용 표시
        rank= FuncDB.rank(self, self.resdefault)

        ee={}
        for i in range(len(rank) // 3):
            i *= 3
            ee[rank[i]] = [rank[i], rank[i + 1], rank[i + 2]]
        for i in range(len(self.resdefault)):
            self.resultTreeview.insertRows(self.resultTreeview.rowCount(), 1)
            for k in range(len(rank)):#처음 5개 항목은 resdefault데이터, 남은 2개 항목은 reswish데이터
                self.resultTreeview.setData(self.resultTreeview.index(i, 0), self.resultTreeview.rowCount())
                #self.resultTreeview.setData(self.resultTreeview.index(i, k + 1), rank[k])
                print(list(ee.values())[k])
                self.resultTreeview.setData(self.resultTreeview.index(i, k + 1), list(ee.values())[k])

        self.textTreeview.setModel(self.resultTreeview)  # 트리뷰위젯에 resultTreeview에서 설정한 내용들을 세팅
        rankLayout.addWidget(self.textTreeview, 1, 0)
...
if __name__ == '__main__':
    ...
    inbody.show()
    #window = MyWindow()
    #window.show() 일단 여기 쓰면 만든 게 뜨긴합니다
    sys.exit(app.exec_())
