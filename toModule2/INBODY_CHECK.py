...
class INBODY(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.resultD.setMaximumHeight(50000)
        outputLayout.addWidget(self.resultD, 1, 0)

        # QDialog 세팅 --> 팝업창(새창) 부분
        ...
        self.rankB.clicked.connect(self.rankOPEN)
    ...
    def rankOPEN(self): #rank 누르면 실행
        ...
        for i in range(len(RankHeader)):
            self.rankWindow.resultRank.setHeaderData(i, Qt.Horizontal, RankHeader[i])

        rank = self.funcDB.rank(self.resdefault)
        rank.sort(key=lambda x:x[1][0], reverse = True) #리스트 안의 요소의 형태인 ['Bicycle ergometer : 100 Watt', '2!', '14.285714285714285%'] 에서 2!의 2를 기준으로 정렬(내림차순)
        for i in range(len(rank)):
            self.rankWindow.resultRank.insertRows(self.rankWindow.resultRank.rowCount(), 1)
            for k in range(3):  # 처음 5개 항목은 resdefault데이터, 남은 2개 항목은 reswish데이터
                self.rankWindow.resultRank.setData(self.rankWindow.resultRank.index(i, 0), self.rankWindow.resultRank.rowCount())
